...
public class TypeChecker implements AbstractVisitor<BType, BType> {
	...
	public BType visitExprOperatorNode(ExpressionOperatorNode node, BType expected) {
		...
		case ID: {
			SetType setType = (SetType) unify(expected, new SetType(new CoupleType(new UntypedType(), new UntypedType())), node);
			visitExprNode(expressionNodes.get(0), new SetType(((CoupleType) setType.getSubType()).getLeft()));
			return node.getType();
		}
		case CLOSURE:
		case CLOSURE1:
